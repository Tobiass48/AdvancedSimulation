...
class BangladeshModel(Model):
    ...
    def __init__(self, scenario_id, seed=None, x_max=500, y_max=500, x_min=0, y_min=0):
        ...
        super().__init__(seed, x_max, y_max, x_min, y_min)  # Call the parent Model's constructor
    ...
    def generate_model(self):
        ...
        df = pd.read_csv('EPA133a-Lab_william/EPA133a-Gxx-A2/data/demo-1.csv')
        ...
        for df in df_objects_all:
            for _, row in df.iterrows():    # index, row in ...
                ...
                if model_type == 'source':
                    agent = Source(row['id'], self, row['length'], row['name'], row['road'], row['condition'])
                    self.sources.append(agent.unique_id)
                elif model_type == 'sink':
                    agent = Sink(row['id'], self, row['length'], row['name'], row['road'], row['condition'])
                    self.sinks.append(agent.unique_id)
                elif model_type == 'sourcesink':
                    agent = SourceSink(row['id'], self, row['length'], row['name'], row['road'], row['condition'])
                    self.sources.append(agent.unique_id)
                    self.sinks.append(agent.unique_id)
                elif model_type == 'bridge':
                    agent = Bridge(row['id'], self, row['length'], row['name'], row['road'], row['condition'])
                elif model_type == 'link':
                    agent = Link(row['id'], self, row['length'], row['name'], row['road'], row['condition'])
